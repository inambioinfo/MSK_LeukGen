...
foreach (@samples)
{
	...
	if($userword==1){
		...
		print "Using rnaseqFastqInput_v1.0.pl...\n";
		system('bsub','-J',$samp,'-oo',"$param{'LOGDIR'}/".$samp."_rnaseqFastqInput_v1.0_".getLoggingTime().".out",'-eo',"$param{'LOGDIR'}/".$samp."_rnaseqFastqInput_v1.0_".getLoggingTime().".err",'perl','rnaseqFastqInput_v1.0.pl',%param,$genome,$samp);
	} elsif($userword==2){
